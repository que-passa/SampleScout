...
| Bottom bar clusters | `.bar-cluster-tight` (4px gap — icon/compact toolbars), `.bar-cluster` (8px — distinct actions on one side), `.bar-actions` (8px — bar-level row); hero-centered rows (Capture record, Take transport) use padding toward the center control, not flex gap                                                                |
...
| Ghost     | **BackButton look is the Ghost style** — flat transparent idle; hover / sticky on = `--surface` face only (no outer well ring); pressed = root/well/face all `--surface` with concentric outer radius (`face radius + well padding`, no inset shadows). Designed to sit on `--paper` (top bars, sheet headers) so the `--surface` face reads; do not place on `--surface` or hover/press vanish. `GhostButton` is the single chrome source; `BackButton` is a thin `GhostButton` link wrapper (`icon` + `href` + back glyph). Optional `compact` (~30px) for waveform toolbar; `danger` = same ghost with `--signal` label/icon only. Use for Back, nav chrome (Account, **Send feedback**, Collection shortcut, Loop), Collection **Select** (text) / **Import** + **Cleanup** (icons) / select-mode actions, take-editor **Reset**, **Field Notes**, sheet close, cancel, and destructive confirm labels. |
...
| Idle plot | Capture stage above the record band shows a standby frame (zero axis, edge ticks, `STANDBY` / `NO MIC` in timer header slot, slow right→left scan) sharing live-wave geometry — not a decorative or fake waveform; `NO MIC` when capabilities say record is unavailable; scan off under `prefers-reduced-motion`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Face     | Treatment                                                                                                                                     |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Local    | `--signal` outline + label (`Local`; device-only meaning unchanged)                                                                           |
...
Accurate PCM-derived peaks only. Canvas + DPR. Clear zero axis and time ruler. Never decorative placeholders, mirrored blobs, EQ bars, or fake paths after audio is loaded. Trim boundary strokes and DOM grips use `--signal` when idle (block tabs below the wave) and stay visible even when retained bounds match the full take; **while dragging a trim edge**, that grip tab and its canvas boundary stroke use `--brand` (same as action toasts / selection), and fade grips soften to 40% transparent. Selection uses `--brand-soft` fill under peaks and `--brand` edges/grips (distinct from idle signal trim); retained ink peaks draw on top of the wash. Fade grips use `--ink` wedge tabs on the bottom edge of the time ruler (ruler includes bottom pad for them); fade envelopes are dashed `--ink` diagonals (not signal). Fades scale drawn peak amplitude by the linear fade gain. Peak-normalize preview gain applies across the full visible wave so edge-finding stays continuous; discarded (outside trim/selection) regions use a `--disabled` wash with `--disabled` peaks (color marks exclusion, not a different amplitude scale); retained audio stays normal `--ink` on paper. Details: [`briefing/04_WAVEFORM_AND_AUDIO_EDITOR.md`](briefing/04_WAVEFORM_AND_AUDIO_EDITOR.md) and `.cursor/rules/waveform-ui.mdc`.
