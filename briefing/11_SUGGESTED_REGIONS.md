...
| 7   | Chrome labels            | Action-row **left**: `collection` icon + **`N scouted`** (singular `1 scouted`). After engage: zero-padded **`01/10`**. **Previous** + **Next** appear only after engage. No active fill on the count control.                                                                                                                                                                   |
...
4. If analysis finishes with **N ≥ 1**: show action-row **left** chrome — `collection` icon + **`N scouted`**. **No Previous/Next yet. Do not** auto-select on load.
5. User taps the scouted control → select suggestion `0` (fit view), switch label to **`01/N`**, reveal **Previous** + **Next**, and show muted scouted markers on the wave + navigator. Further taps on the control **exit** scouted mode (hide nav + markers; selection remains as last applied until the user edits it).
6. **Next** / **Previous** advance through suggestions (wrap; label updates e.g. `02/10`).
...
| Auto budget           | aim ≲ 300 ms Worker time typical | First auto exceed → manual-only Analyze for rest of browser session (not persisted; cache still used; Manual Analyze when duration gate passes). Hard timeout stays separate (`12s`). |
...
| N ≥ 1          | Icon + count; **Previous** + **Next** only after engage                                                                     |
...
- `collection` icon + **`N scouted`** only (no Previous/Next, no wave markers)

After user taps scouted:

- Label becomes **`01/10`** (zero-padded) + **Previous** + **Next**
...
- Real buttons; labels like “Previous suggested region”, “Next suggested region”, “3 suggested regions”, “Analyze scouted regions”.
...
- Visible chrome uses “scouted”; `aria-label`s may use “suggested regions” for fuller SR meaning.
- No active fill / `aria-pressed` sticky on the count control (engaged state is `01/N` + Previous/Next + markers).
...
3. **Chrome clutter** — Action-row left only; Previous/Next hidden until engage; hide when N = 0.
...
- Transport left: icon + count + Previous/Next (after engage); hide when empty.
...
- Timing vs 300 ms budget; first session exceed switches remaining opens to manual-only Analyze (landed).
...
- Previous/Next cycle (wrap); icon + count reflect N; chrome hidden when N = 0 or ineligible.
