# ADR 0003 — Collect retained trim as Local File (shared source)
...
The product brief originally centered on many short, independent takes. Field practice more often captures a longer recording that contains several useful samples (atmo, textures, hits in sequence). Trim/Cut only reshape **one** take’s recipe and still yield a single upload unit. Users need to carve multiple upload-ready Local Files from one parent without DAW-style multi-region export.
...
- Do not build a multi-region marker list or multi-upload-from-one-take job model for MVP; each collected file is a normal take with normal upload.
...
- Implementation: `collectableRetainedBounds` / `buildExtractTake` / collect persistence helpers / take **Collect** primary gated on trim; shared `fileRef`, `derivedFromTakeId`, refcounted cleanup in `filterUnheldFileRefs` / `processDueCleanups`.
